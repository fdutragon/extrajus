        ...
        if (userData?.user?.email && docData) {
          ...
          if (!docData.is_paid) {
            ...
            if (resendKey && docData.content) {
              ...
              if (sendResult.error) {
                console.error("[Polling Status] Falha no envio de e-mail:", sendResult.error);
              } else {
                console.log(`[Polling Status] E-mail enviado com sucesso para ${toEmail}`);
              }
              }
            }
          }
    ...
    if (externalId.startsWith("paydoc_")) {
      ...
      try {
        ...
        if (resendKey && userData?.user?.email && docData?.content) {
          ...
          if (sendResult.error) {
            console.error("[Simulação Billing Status] Falha no envio de e-mail:", sendResult.error);
          } else {
            console.log(`[Simulação Billing Status] E-mail enviado com sucesso para ${toEmail}`);
          }
        }
      } catch (emailErr: any) {
